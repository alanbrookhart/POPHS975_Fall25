---
title: "Estimating causal effects with propensity scores"
...
tidy(ps_model, exponentiate = TRUE, conf.int = TRUE) %>% kable()
```
...
ns_analy = ns %>% mutate(ps = predict(ps_model, type = "response"))
```
...
ns_analy <- ns_analy %>% mutate(iptw = ifelse(cox2_initiation == "Yes", 1/ps, 1/(1-ps)))
...
ns_analy %>% filter(cox2_initiation == "Yes") %>% pull(iptw) %>% summary() 
...
```{r}
glm_weighted = glm(incident_pud ~ cox2_initiation, data = ns_analy, family = binomial(link = "logit"),
                     weights = iptw)
